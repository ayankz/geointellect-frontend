...
export class HintComponent {
  ...
  public cities = [
    ...
    {
      name: 'Шымкент',
      center: [ 42.318104777,69.59311980 ],
      active: false
    }
  ]
  setActiveCity(event: number): void{
    ...
    this.mapService.setCenter(this.cities[event].center);
  ...
  next(step:number){
    ...
    if (this.step === 2) {
      ...
      this.hint.nativeElement.children[2].innerHTML = 'Выберите необходимые критерии для анализа локации.';
    } else if (this.step === 3) {
      this.hint.nativeElement.setAttribute('style', 'display: none');
